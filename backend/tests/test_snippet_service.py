...
async def test_save_snippet_is_idempotent(db_session: AsyncSession, current_user: User) -> None:
    ...
    snippet = await _make_snippet(db_session, topic.id)
    await db_session.commit()
    # Capture ids before save_snippet's rollback path expires the session.
    user_id = current_user.id
    snippet_id = snippet.id

    from app.services.snippet_service import save_snippet

    await save_snippet(db_session, snippet_id, current_user)
    await save_snippet(db_session, snippet_id, current_user)  # second call must not raise
    ...
    count = (
        await db_session.execute(
            select(func.count()).where(
                UserSnippetSave.user_id == user_id,
                UserSnippetSave.content_id == snippet_id,
            )
        )
    ).scalar_one()
    assert count == 1
